...
Every mobile-mutable property, season, field, ecological site, observation,
media record, and operational alert is schema-validated before entering the
shared change feed. IDs, bounded closed map geometry, coordinates, timestamps,
domain enums, evidence digests, depth metadata, and privacy-bounded camera
capture evidence must match the mobile contracts. Raw EXIF and unknown fields
are rejected. Legacy media without the optional camera evidence object remains
readable and synchronizable.
